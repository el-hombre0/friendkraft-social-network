...
if (isset($_POST)) {
    if (empty($_POST['email'])) {
        ...
        if (pg_num_rows($sql) > 0) {
            ...
            $q = "INSERT INTO users(email, password, data, ip, activation)VALUES('$email','$password', '$data', '$ip', '1')";
            ...
            перейдите по ссылке: \n http://www.friendkraft.bget.ru/index?email=" . $email . "&act=" . $activation . "\n\n
            С уважением администрация сайта Friendkraft.ru ";
            mail($email, $subject, $message, "Content-type:text/plane Charset=utf-8\n\n");
            exit("<b>Вы успешно зарегистрировались, на ваш E-mail отправленна ссылка для активации вашего аккаунта!</b>");
        }
    }
}
?>